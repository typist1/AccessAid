...
export default function Signup() {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-2xl font-bold text-white mb-2">{t('auth.signup_title')}</h1>
      <p className="mb-6 font-medium text-white">{t('auth.signup_subtitle')}</p>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <Input label={t('auth.password_label')} type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} labelClassName="text-white text-base font-semibold" />
      ...
      <p className="text-center text-sm mt-6 font-medium text-white">
        {t('auth.have_account')}{' '}
        <Link to="/login" className="font-semibold underline underline-offset-2 text-white hover:text-purple-200">{t('auth.sign_in_link')}</Link>
      </p>
    </div>
  )
}
